...
def setup_rq_connection():
    ...
    if isinstance(redis_url, list):
        ...
        sentinel_hosts = [
            tuple(sentinel.split(":", 1)) for sentinel in redis_sentinels.split(",")
        ]
...
def serialize_queues(queues):
    return [
        dict(name=q.name, count=q.count, url=url_for(".overview", queue_name=q.name))
        for q in queues
    ]
...
def list_jobs(queue_name, page):
    ...
    last_page_link = dict(
        url=url_for(".overview", queue_name=queue_name, page=last_page)
    )
...
def serialize_current_job(job):
    ...
    return dict(
        job_id=job.id,
        ####################################
        # divvyDOSE addition of job_origin.
        ####################################
